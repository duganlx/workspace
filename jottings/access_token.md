...
目前的设计实体中有用户 USER、用户组 USERGROUP、资源组 SRCGROUP、资源 SRC，其中资源 SRC 对应是业务中的服务。因为最新的需求中服务下需要细分模块，所以将资源实体的命名格式进行调整，原本的 `xxx` 变为 `xxx/module1`，即用 `/` 进行划分模块。所以目前的策略规则命名规则如下。
